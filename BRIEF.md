...
Include Rectangle's MIT notice in `THIRD_PARTY_NOTICES`. We write fresh Python;
no Swift is copied.
...
| 3 | First / Center / Last third | `Ctrl+Alt+D/F/G` |
| 4 | First / Last two-thirds | `Ctrl+Alt+E/T` |
| 5 | Maximize | `Ctrl+Alt+Enter` |
| 6 | Maximize height | `Ctrl+Alt+Shift+↑` |
| 7 | Almost maximize (~85%) | `Ctrl+Alt+Shift+Enter` |
| 8 | Center (no resize) | `Ctrl+Alt+C` |
| 9 | Make larger / smaller | `Ctrl+Alt+=` / `Ctrl+Alt+-` |
| 10 | Restore (undo) | `Ctrl+Alt+Backspace` |
| 11 | Move to next / prev display | `Ctrl+Alt+Cmd+←/→` → `Ctrl+Alt+,` / `Ctrl+Alt+.` (avoid Win+arrow, reserved by OS Snap) |
| 12 | Next-third cycle (repeat key cycles) | repeated half/third presses |
| 13 | **Drag-to-edge snapping** (footprint preview) | mouse to screen edge/corner |
| 14 | Configurable **gap** between windows | n/a (setting) |
| 15 | System-tray menu + preferences window | n/a |
| 16 | Launch at login | n/a |
