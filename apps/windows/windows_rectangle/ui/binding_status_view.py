...
def binding_status_text(report: BindingReport | None) -> str:
    ...
    summary = f"{report.bound_count} of {report.total} shortcuts bound" + (
...
def binding_status_html(report: BindingReport | None) -> str:
    ...
    paused = report.paused
    parts: list[str] = []
    summary = f"<p><b>{report.bound_count}</b> of <b>{report.total}</b> shortcuts bound"
    if paused:
    ...
    if report.bound:
        rows = "".join(
            f"<tr><td>{html.escape(a.value)}</td><td><kbd>{html.escape(c)}</kbd></td></tr>"
