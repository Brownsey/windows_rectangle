...
MIN_ALMOST_MAXIMIZE_SCALE = 0.5
MAX_ALMOST_MAXIMIZE_SCALE = 1.0
...
def almost_maximize(
    ...
    *,
    ...
    scale = clamp_almost_maximize_scale(scale)
...
def clamp_almost_maximize_scale(scale: float) -> float:
    return max(MIN_ALMOST_MAXIMIZE_SCALE, min(MAX_ALMOST_MAXIMIZE_SCALE, float(scale)))
...
def apply(
    ...
    *,
    ...
    if action is Action.ALMOST_MAXIMIZE:
