...
from .eligibility import WindowFlags
...
class WorkspaceWindows(Protocol):
    ...
    def get_window_flags(self, handle: object) -> WindowFlags: ...
...
def capture_workspace(manager: WorkspaceWindows, name: str) -> Workspace:
    ...
    for window in manager.list_windows():
        # Never save this utility's own preferences/editor/child dialogs.
        # The title check keeps compatibility with non-Win32 test adapters;
        # process ownership catches dialogs with generic titles such as Add App.
        owned_by_this_process = window.process_id == os.getpid()
        legacy_title_match = window.process_id is None and window.title.casefold().startswith(
            "windows rectangle"
        )
        if owned_by_this_process or legacy_title_match:
            continue
...
def apply_workspace(manager: WorkspaceWindows, workspace: Workspace) -> WorkspaceResult:
    ...
    work_areas = manager.list_work_areas()
    plan = plan_workspace(workspace, manager.list_windows(), work_areas)
    ...
    for placement in workspace.placements:
        if placement.monitor_index >= len(work_areas):
            results.append(
                PlacementResult(
                    placement.id,
                    "blocked",
                    f"Display {placement.monitor_index + 1} is not connected",
                )
            )
            continue
        ...
        try:
            if (
                manager.is_maximized(move.handle)
                or manager.get_window_flags(move.handle).is_minimized
            ):
...
def launch_and_apply_workspace(
    ...
    *,
    ...
    initial = match_workspace_windows(workspace.placements, manager.list_windows())
    matched = {match.placement_id for match in initial.matches}
    ...
    if launched and timeout > 0:
        ...
        while monotonic() < deadline:
            current = match_workspace_windows(workspace.placements, manager.list_windows())
            current_matches = {match.placement_id for match in current.matches}
