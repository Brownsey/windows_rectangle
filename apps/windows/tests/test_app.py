...
def _enabled_default_count() -> int:
    return sum(1 for combo in DEFAULT_SHORTCUTS.values() if combo)
...
def test_build_sets_cycle_idle_timeout(windows):
    ...
    assert ctx.dispatcher.cycle_idle_timeout == 3.0


def test_build_sets_almost_maximize_scale(windows):
    ctx = build(Settings(almost_maximize_scale=0.75), windows)
    assert ctx.dispatcher.almost_maximize_scale == 0.75
...
def test_apply_settings_updates_cycle_timeout(windows):
    ...
    assert ctx.dispatcher.cycle_idle_timeout == 5.5


def test_apply_settings_updates_almost_maximize_scale(windows):
    ctx = build(Settings(almost_maximize_scale=0.85), windows)
    ctx.apply_settings(Settings(almost_maximize_scale=0.70))
    assert ctx.dispatcher.almost_maximize_scale == 0.70


def test_apply_settings_clamps_almost_maximize_scale(windows):
    ctx = build(Settings(almost_maximize_scale=0.85), windows)
    ctx.apply_settings(Settings(almost_maximize_scale=2.0))
    assert ctx.dispatcher.almost_maximize_scale == 1.0


def test_dispatcher_uses_configured_almost_maximize_scale(windows):
    ctx = build(Settings(almost_maximize_scale=0.75), windows)
    result = ctx.dispatcher.dispatch(Action.ALMOST_MAXIMIZE)
    assert result.moved
    assert result.after == Rect(240, 130, 1440, 780)
...
def test_bind_hotkeys_registers_every_action(windows):
    ...
    assert bound == _enabled_default_count()
    assert len(hot.registered) == _enabled_default_count()


def test_bind_hotkeys_skips_blank_disabled_commands(windows):
    hot = FakeHotkeys()
    settings = Settings()
    settings.shortcuts[Action.LEFT_HALF] = ""
    ctx = build(settings, windows, hotkeys=hot)

    bound = bind_hotkeys(ctx, hot.register)

    assert bound == _enabled_default_count() - 1
    assert all(combo for combo, _callback in hot.registered.values())
    assert DEFAULT_SHORTCUTS[Action.LEFT_HALF] not in {
        combo for combo, _callback in hot.registered.values()
    }
...
def test_bind_hotkeys_via_bus_skips_blank_disabled_commands(windows):
    hot = FakeHotkeys()
    settings = Settings()
    settings.shortcuts[Action.LEFT_HALF] = ""
    ctx = build(settings, windows, hotkeys=hot)
...
def test_drain_actions_limits_work_per_tick(windows):
    ctx = build(Settings(), windows)
    ctx.bus.submit(Action.LEFT_HALF)
    ctx.bus.submit(Action.RIGHT_HALF)
    ctx.bus.submit(Action.MAXIMIZE)

    count = ctx.drain_actions()

    assert count == 1
    assert ctx.bus.pending() == 2


def test_drain_actions_trims_stale_shortcut_storm(windows):
    ctx = build(Settings(), windows)
    for _ in range(40):
        ctx.bus.submit(Action.CENTER)

    count = ctx.drain_actions()

    assert count == 1
    assert ctx.bus.pending() == 23


def test_rebind_hotkeys_refreshes_registered_combos(windows):
    hot = FakeHotkeys()
    ctx = build(Settings(), windows, hotkeys=hot)
    bind_hotkeys_via_bus(ctx, hot.register)

    updated = Settings()
    updated.shortcuts[Action.LEFT_HALF] = "ctrl+alt+h"
    ctx.apply_settings(updated)
    bound = ctx.rebind_hotkeys()

    assert bound == _enabled_default_count()
    assert hot.unregister_all_calls == 1
    assert any(combo == "ctrl+alt+h" for combo, _ in hot.registered.values())
